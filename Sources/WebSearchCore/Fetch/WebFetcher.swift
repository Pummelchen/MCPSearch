...
public actor WebFetcher {
    ...
    private func performOpen(_ request: FetchRequest) async throws -> FetchResult {
        ...
        guard let jina else {
            ...
            // not tell "this page has no text" from "every extraction path failed".
        ...
        do {
            ...
            // the caller should see rather than a success carrying nothing.
            guard let result = directResult, !result.text.isEmpty else {
                throw directError ?? SearchError.extractionFailed(request.url)
            }
            var finalized = result
    ...
    /// caller went away rather than that the request failed.
