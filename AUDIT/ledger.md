...
- **Evidence after:** Committed in 08a52b4: steps 1 and 2 of six. The bypass is refused and the model is measured against the parsed tree. Soundness: no case under-counts. Tightness: exact (0) when end tags are explicit, +2 to +4 when they are omitted — `<p>one<p>two` is ordinary HTML, and that over-count accumulates with every omitted tag toward the 512 limit. REMAINING: step 3 (the optional-end-tag rules: `p` closed by the block set, `li` by `li`, `dt`/`dd` by each other, `option`/`optgroup`, `tr`, `td`/`th`, table sections), then the broadened harness, the threshold choice from that evidence, and making our own `walk` iterative at HTMLExtractor.swift. Suite green at 617 tests (575 + 42), 0 failures; both linters exit 0.
- **Commit:** `08a52b4`
