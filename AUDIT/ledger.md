...
- **Evidence after:** Committed in 5e724c9: steps 1-3 of six. The bypass is refused; the model matches the parsed tree exactly on all eight measured constructs, including the omitted end tags HTML permits (before step 3 the same rows over-counted by +2 to +4, which accumulates toward the 512 limit on a long page). One under-count was introduced and fixed while doing it: `<tr>` with no open section gets an implicit `<tbody>` from the parser, so not modelling it read 5 where the tree was 6. REMAINING: step 4 (broaden the corpus — adoption-agency elements, select-in-table, foreign content — so tightness stops being an assertion), step 5 (choose the threshold from that evidence), step 6 (make our own `walk` iterative at HTMLExtractor.swift, necessary though not sufficient). Suite green at 619 tests (577 + 42) — the commit message says 618 (576 + 42), which is wrong; the run reports 577 core tests, and the correct total is recorded here rather than repeated, 0 failures; both linters exit 0.
- **Commit:** `5e724c9`
