...
- **Evidence after:** REOPENED AND RE-CLOSED in 14087ab. The first closure rested on a corpus of 25 hand-written constructs and claimed the model 'exact on 24 of 25'. That was not evidence: a hand-written construct is a document someone had in mind while writing the model. Measured against 37 REAL pages fetched from the web, the model read shallower than the parser on 18 of them — the bypass direction — so the closure was premature. Two independent defects, neither visible in any hand-written case: (1) the implied-end-tag walks crossed their own scope, so `li`-closes-`li` popped through the enclosing `<ul>` and `ul > li > ul > li > ul > li` lost two levels; fixed by carrying the spec's scope boundaries (the list, the definition list, a table cell, a template), where anything omitted can only over-count. A first attempt also bounded the EXPLICIT end-tag walk by a set containing `ul`/`dl`/`td`, making those elements unclosable and over-counting 35 of 37 pages, one by 560 levels. (2) void and foreign-content self-closing elements were never counted as a level at all, so every `<circle/>`, `<br>` and `<img>` was a level the parser built and the model did not; bisected to the character (`<line/>` in an SVG icon). After both fixes: 34 of 37 real pages exact, 3 over, 0 under; the over-counts are +14/+2/+1, the safe direction, against a limit of 512. The real-page comparison is an opt-in test (`MARKUP_DEPTH_CORPUS`), so the default suite stays hermetic. The hermetic corpus is 37 cases including four that pin the void and self-closing behaviour. Suite green at 582 tests, 0 failures; both linters exit 0.
- **Commit:** `14087ab`
